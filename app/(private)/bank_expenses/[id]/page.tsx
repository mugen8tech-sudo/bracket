...
export default function BankExpenseDetailPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const [bRes, pRes] = await Promise.all([
        supabase.from("banks").select("id, bank_code, account_name, account_no").eq("id", r.bank_id).maybeSingle(),
        supabase.from("profiles").select("user_id, full_name").eq("user_id", r.created_by).maybeSingle(),
      ]);
  ...
  if (!row) {
    return (
      <div className="space-y-3">
        <div className="rounded border bg-white p-3"><b>Expense Information</b></div>
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="rounded border bg-white p-0 overflow-hidden">
        <table className="w-full">
          <tbody>
            ...
            <tr>
              <td className="p-3 border-b bg-gray-50">Category</td>
              <td className="p-3 border-b">{row.category_code}</td>
            </tr>
            <tr>
      ...
      <Link href="/bank_expenses" className="rounded bg-gray-100 px-4 py-2 inline-block">Back</Link>
    </div>
  );
}
